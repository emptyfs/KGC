...
df = pd.read_csv("src/web_parsing_and_fake_news_detection/third_dataset/fake_or_real_news.csv")

df_fake = df[df['label'] == 'FAKE']
df_true = df[df['label'] == 'REAL']

fake_articles = [[row['title'], row['text']] for index, row in df_fake.iterrows()]
true_articles = [[row['title'], row['text']] for index, row in df_true.iterrows()]
